...
def generate_report(results: dict, output_path: str) -> str:
    ...
    elements.append(Paragraph(
        f"<font size=32 color='{BRAND_COLOR.hexval()}'><b>⚡ {BRAND_NAME}</b></font>",
        ParagraphStyle('cover_brand', alignment=TA_CENTER, spaceAfter=4)))
    elements.append(Paragraph(
        "<font size=14 color='#6b7280'>SECURITY ASSESSMENT REPORT</font>",
        ParagraphStyle('cover_sub', alignment=TA_CENTER, spaceAfter=30)))

    elements.append(HRFlowable(width="80%", thickness=2, color=BRAND_COLOR,
                                spaceAfter=20, hAlign='CENTER'))

    # Grade preview on cover
    elements.append(Paragraph(
        f"<font size=72 color='{grade_info['color'].hexval()}'><b>{grade_info['grade']}</b></font>",
        ParagraphStyle('cover_grade', alignment=TA_CENTER, spaceAfter=4, leading=80)))
    elements.append(Paragraph(
        f"<font size=16><b>{grade_info['label']}</b></font>",
        ParagraphStyle('cover_label', alignment=TA_CENTER, spaceAfter=30)))

    # Meta info on cover
    meta_data = [
        ['Target Domain', domain],
        ['Report Date', scan_date.strftime('%B %d, %Y · %H:%M UTC')],
        ['Report ID', report_id],
        ['Classification', 'CONFIDENTIAL'],
        ['Security Grade', f"{grade_info['grade']} ({grade_info['label']})"],
        ['Risk Score', f"{risk_score}/100"],
    ]
    mt = Table(meta_data, colWidths=[2*inch, 4*inch])
    ...
    rec_data = [
        ...
        ['🔴 P1', 'Remediate all Critical and High severity findings', 'Within 48 hours'],
        ['🟠 P2', 'Patch known CVEs on all live hosts', 'Within 1 week'],
        ['🟡 P3', 'Implement missing security headers (CSP, HSTS, X-Frame-Options)', 'Within 2 weeks'],
        ['🔵 P4', 'Upgrade deprecated TLS/SSL configurations', 'Within 1 month'],
        ['⚪ P5', 'Conduct follow-up scan to verify remediation and track grade improvement', 'Monthly'],
    ]
    rt = Table(rec_data, colWidths=[0.8*inch, 3.8*inch, 1.7*inch])
